...
| Layer | Question | Answer |
|-------|----------|--------|
| **Plain English** | What problem does this solve? | A malicious or compromised tool in an MCP registry can hide instructions in its description field — the LLM reads them as instructions, the human never sees them. The defense is to validate every tool description before it reaches the LLM: hash-check your own descriptions for tampering, scan all descriptions for injection patterns, allowlist any external tool registries. |
| **System Role** | Where does it sit in AOIS? | At tool registration time in `server.py`. `scan_description()` is called for every tool description at server startup. `verify_tool_description()` checks known-good hashes. Any description that fails is a startup error — the server does not start with poisoned tools. For external tools (v21 multi-server orchestration), `ALLOWED_EXTERNAL_TOOLS` allowlist is checked before any remote tool is loaded. |
| **Technical** | What is it, precisely? | Three-layer defense: (1) Regex-based injection scanning — patterns like `"IMPORTANT SYSTEM NOTE"`, `"do not mention this"`, exfiltration URL patterns; (2) SHA-256 hash verification for your own tool descriptions — if the description changes between deployments without a code change, the hash fails; (3) Registry allowlisting — external tool sources are explicitly enumerated, all others are rejected. Confirmed via Cisco research (2026) against a real third-party MCP skill that exfiltrated data silently. |
| **Remove it** | What breaks, and how fast? | A compromised third-party MCP tool can instruct AOIS to exfiltrate conversation context, manipulate Mem0 memory, or re-route investigations to attacker-controlled tools. The attack bypasses OAuth (attacker has a valid token), sandboxing (tool runs in sandbox and still exfiltrates), and rate limiting (one call is enough). Detection without the defense: only in OTel traces after the fact, if you notice anomalous tool calls. |
