...
| Layer | Question | Answer |
|---|---|---|
| **Plain English** | What problem does this solve? | Agents finish their work before the user sees any of it — the UI shows a spinner for 45 seconds then a result. AG-UI is a standard event stream so the frontend renders each tool call, each reasoning step, and each finding as the agent produces it. The user watches the investigation happen live. |
| **System Role** | Where does it sit in AOIS? | AOIS emits AG-UI events from the `/investigate/stream` SSE endpoint. The v26 React dashboard subscribes with `EventSource`. Every `ToolCallStart`, `TextMessageChunk`, and `StateSnapshot` the agent fires is rendered in the UI in real time — tool-call cards, streaming text, severity chips. |
| **Technical** | What is it, precisely? | An event-based protocol over Server-Sent Events (SSE), published by CopilotKit in May 2025. Defines typed events: `RunStarted`, `TextMessageStart/Chunk/End`, `ToolCallStart/End`, `StateSnapshot`, `StateDelta`, `RunFinished`, `RunError`. Each event is a JSON object in a standard `data: {...}\n\n` SSE frame. The connection stays open; events push as they occur. Bidirectional: the frontend can also send messages upstream via a separate POST. |
| **Remove it** | What breaks, and how fast? | Remove AG-UI → the dashboard returns to polling or waiting for final results. No real-time tool-call visibility, no streaming text, no mid-investigation state. Users see a spinner instead of a live investigation. AWS Bedrock AgentCore's native streaming support cannot be used. The v26 dashboard degrades to a results viewer rather than a live SRE tool. |
